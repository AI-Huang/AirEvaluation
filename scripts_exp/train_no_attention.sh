...
attention_type="official" # "MyAttention", --no-attention
...
for stride in $hparam_stride; do
    ...
    for sensor_index in $hparam_sensor_index; do
        echo "sensor_index: $sensor_index; stride: $stride; --no-attention"
        python train.py --model_name "WaveNet_LSTM" --no-attention --data_type=pm25 --sensor_index=$sensor_index --shuffle=True --stride=$stride --output_dir="./output"
