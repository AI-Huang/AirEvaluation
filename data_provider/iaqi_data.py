...
import numpy as np
import pandas as pd
from data_provider.window_sequences import WindowSequences, MultiChannelWindowSequences
...
def load_data(data_name, window_size, stride, batch_size=32, validation_split=0, shuffle=True, seed=42, standard="USA"):
    ...
    df_pm_iaqi = pd.read_pickle(
        f"./datasets/iaqi_data/{date}/{data_type}_iaqi_{standard}.pkl")
    df_pm_level_polygonal = pd.read_pickle(
        f"./datasets/iaqi_data/{date}/{data_type}_level_polygonal_{standard}.pkl")
